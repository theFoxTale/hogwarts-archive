...
- [x] **(+14)** Statement coverage ≥80%, branch, function, line coverage ≥50% (подтверждено отчётом).  
      `statements: 85.2%, branches: 72.5%, functions: 68.4%, lines: 85.2%`
...
- [x] **(-0)** Покрытие соответствует порогам (statement 85%).
...
- **Изображение** – если `image` не указан, используется заглушка `anonymous.jpg`; кастомное изображение подставляется корректно.
...
#### 5. FloatingErrorButton (или ErrorButton)
...
- **Выполнение поиска** – по клику на "Search" отправляется запрос с обрезанным термином, данные сохраняются в `localStorage`.
...
- **Пагинация** – переход на следующую страницу: отправляется запрос с текущим поисковым термином и новым номером страницы, номер страницы сохраняется в `localStorage`.

#### 7. API mocking (общие сценарии)

- **Успешный ответ** – мок возвращает предопределённые данные, компоненты рендерят их.
- **Ошибка сети / сервера** – мок возвращает rejected promise, отображается пользовательское сообщение.
- **Пустой ответ** – мок возвращает `{ items: [], pages: null }`, показывается "No characters found".
...
Все пороги (`statements ≥80%`, остальные ≥50%) достигнуты.

---

### 🔧 Дополнительные проверки (выполнено)

- [x] Все тесты проходят: `npm run test` → ✅ 28 passed, 0 failed.
...
- [x] Отчёт о покрытии прикреплён к PR (скриншот).
